...
and generating overlays that show only retained objects.
...
for fname in os.listdir(mask_dir):
    ...
    for label in labels:
        ...
        # compute means per channel (robust to grayscale)
        mean_rgb = [np.nan, np.nan, np.nan]
        if image.ndim == 2:
            # grayscale -> same mean in all channels
            mean_val = float(p.mean_intensity)
            mean_rgb = [mean_val, mean_val, mean_val]
        else:
            # regionprops w/ multichannel intensity returns array per pixel;
            # easier: mask the image and compute means directly
            coords = p.coords
            pix = image[coords[:, 0], coords[:, 1]]
            if pix.ndim == 1:  # just in case
                pix = pix[:, None]
...
header = [
    ...
    'Red Avg', 'Green Avg', 'Blue Avg', 'Length', 'Width', 'L/W Ratio',
